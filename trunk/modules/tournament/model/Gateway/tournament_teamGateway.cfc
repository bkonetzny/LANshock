<cfcomponent displayname="tournament_teamGateway.cfc" extends="reactor.project.lanshock.Gateway.tournament_teamGateway">
...
<cfset variables.sOptionNameField = "name">
	...
	<cffunction name="getRecords" access="public" hint="I return all matching rows from the table." output="false" returntype="query">
		...
		<cfif NOT StructIsEmpty(arguments.stFilter)>
			...
			<cfif StructKeyExists(arguments.stFilter,'stJoins')>
				<cfset Query.returnObjectFields("tournament_team","id,name,tournamentid,leaderid,autoacceptids,leagueid")>
				
				<cfloop collection="#arguments.stFilter.stJoins#" item="idx">
					<cfset idx = lCase(idx)>
					<cfset Query.leftJoin("tournament_team",idx,idx)>
			...
			<cfif StructKeyExists(arguments.stFilter,'stFields')>
				<cfloop collection="#arguments.stFilter.stFields#" item="idx">
					...
					<cfelse>
						<cfset sCurrentTable = "tournament_team">
			...
			<cfif StructKeyExists(arguments.stFilter,'lSortFields')>
				<cfloop list="#arguments.stFilter.lSortFields#" index="idx">
					...
					<cfif ListLen(idx,'|') EQ 2>
						<cfset idx = ListPrepend(idx,"tournament_team",'|')>
					</cfif>
	...
	<cffunction name="getAllWithJoin" output="false" returntype="query" hint="I get ALL the records, with an outer join to any parent tables.">
		...
		<cfset Query.leftJoin("tournament_team", "tournament_tournament", "tournament_tournament")/>
		<cfset Query.setFieldPrefix("tournament_tournament","tournament_tournament_")/>
	
		<cfset Query.leftJoin("tournament_team", "user", "user")/>
	...
	<cffunction name="getNWithJoin" output="false" returntype="query" hint="I get the selected N records, with an outer join to any parent tables.">
		...
		<cfif arguments.startrow GT 1>
			...
			<cfset QuerySkip.returnObjectFields("tournament_team","id") />
			<cfset QuerySkip.setMaxrows(arguments.startrow) />
			<cfset qSkip = getByQuery(QuerySkip) />
			<cfset where.isNotIn("tournament_team","id",valuelist(qSkip.id)) />
		</cfif>
	  
		<!--- Left join all the associated tables (uses a left join for safety). --->
		<!--- Syntax is: Query.leftJoin(joinFromObjectAlias,joinToObjectAlias,relationshipAlias) --->
	  
		<cfset QueryRecordset.leftJoin("tournament_team", "tournament_tournament", "tournament_tournament")/>
		<cfset QueryRecordset.setFieldPrefix("tournament_tournament","tournament_tournament_")/>
	  
		<cfset QueryRecordset.leftJoin("tournament_team", "user", "user")/>
		...
		<cfif isStruct(arguments.filter)>
			<cfset QueryRecordset.getWhere().setMode("OR")>
				...
				<cfif StructKeyExists(arguments.filter,'id')>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("tournament_team",'id',arguments.filter['id'])>
					</cfif>
				</cfif>
				...
				<cfif StructKeyExists(arguments.filter,'name')>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("tournament_team",'name',arguments.filter['name'])>
					</cfif>
				</cfif>
				...
				<cfif StructKeyExists(arguments.filter,'tournamentid')>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("tournament_team",'tournamentid',arguments.filter['tournamentid'])>
					</cfif>
				</cfif>
			
				<cfif StructKeyExists(arguments.filter,'leaderid')>
					
							<cfset bFilterColumn = false>
							<cfif isNumeric(arguments.filter['leaderid'])>
								<cfset bFilterColumn = true>
							</cfif>
						
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("tournament_team",'leaderid',arguments.filter['leaderid'])>
					</cfif>
				</cfif>
			
				<cfif StructKeyExists(arguments.filter,'autoacceptids')>
					
							<cfset bFilterColumn = true>
						
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("tournament_team",'autoacceptids',arguments.filter['autoacceptids'])>
					</cfif>
				</cfif>
			
				<cfif StructKeyExists(arguments.filter,'leagueid')>
					
							<cfset bFilterColumn = true>
						
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("tournament_team",'leagueid',arguments.filter['leagueid'])>
					</cfif>
				</cfif>
		...
		<cfset QueryRecordset.returnObjectFields("tournament_team","id,name,tournamentid,leaderid")>
	...
	<cffunction name="getOptions" access="remote" output="false" returntype="query" hint="I get the option values for this table.">
		...
		<cfif len(variables.sOptionNameCode)>
			<cfset QueryRecordset.setFieldExpression("tournament_team",variables.sOptionNameField,variables.sOptionNameCode)>
		</cfif>
		<cfset QueryRecordset.setFieldAlias("tournament_team","id","optionvalue")>
		<cfset QueryRecordset.setFieldAlias("tournament_team",variables.sOptionNameField,"optionname")>
		<cfset QueryRecordset.returnObjectFields("tournament_team","optionvalue,optionname")>
	...
	<cffunction name="getRecordCount" output="false" returntype="numeric" hint="I get the number of records in the table.">
		...
		<cfset QueryCount.returnObjectFields("tournament_team","id") />
		<cfset QueryCount.setFieldExpression("tournament_team","id","COUNT(*)","CF_SQL_INTEGER") />
