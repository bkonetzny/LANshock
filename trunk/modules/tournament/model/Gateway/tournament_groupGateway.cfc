<cfcomponent displayname="tournament_groupGateway.cfc" extends="reactor.project.lanshock.Gateway.tournament_groupGateway">
<!--- Start of gatewayCustom code generated by fusebox scaffolder, it will be replaced if fusebox scaffolder is rerun. --->
<cfset variables.sOptionNameField = "season_id">
	...
	<cffunction name="getRecords" access="public" hint="I return all matching rows from the table." output="false" returntype="query">
		...
		<cfif NOT StructIsEmpty(arguments.stFilter)>
			...
			<cfif StructKeyExists(arguments.stFilter,'stJoins')>
				<cfset Query.returnObjectFields("tournament_group","id,season_id,name,description,maxsignups")>
				
				<cfloop collection="#arguments.stFilter.stJoins#" item="idx">
					<cfset idx = lCase(idx)>
					<cfset Query.leftJoin("tournament_group",idx,idx)>
			...
			<cfif StructKeyExists(arguments.stFilter,'stFields')>
				<cfloop collection="#arguments.stFilter.stFields#" item="idx">
					...
					<cfelse>
						<cfset sCurrentTable = "tournament_group">
			...
			<cfif StructKeyExists(arguments.stFilter,'lSortFields')>
				<cfloop list="#arguments.stFilter.lSortFields#" index="idx">
					...
					<cfif ListLen(idx,'|') EQ 2>
						<cfset idx = ListPrepend(idx,"tournament_group",'|')>
					</cfif>
	...
	<cffunction name="getAllWithJoin" output="false" returntype="query" hint="I get ALL the records, with an outer join to any parent tables.">
		...
		<cfset Query.leftJoin("tournament_group", "tournament_season", "tournament_season")/>
		<cfset Query.setFieldPrefix("tournament_season","tournament_season_")/>
	...
	<cffunction name="getNWithJoin" output="false" returntype="query" hint="I get the selected N records, with an outer join to any parent tables.">
		...
		<cfif arguments.startrow GT 1>
			...
			<cfset QuerySkip.returnObjectFields("tournament_group","id") />
			<cfset QuerySkip.setMaxrows(arguments.startrow) />
			<cfset qSkip = getByQuery(QuerySkip) />
			<cfset where.isNotIn("tournament_group","id",valuelist(qSkip.id)) />
		</cfif>
	  
		<!--- Left join all the associated tables (uses a left join for safety). --->
		<!--- Syntax is: Query.leftJoin(joinFromObjectAlias,joinToObjectAlias,relationshipAlias) --->
	  
		<cfset QueryRecordset.leftJoin("tournament_group", "tournament_season", "tournament_season")/>
		<cfset QueryRecordset.setFieldPrefix("tournament_season","tournament_season_")/>
		...
		<cfif isStruct(arguments.filter)>
			<cfset QueryRecordset.getWhere().setMode("OR")>
				...
				<cfif StructKeyExists(arguments.filter,'id')>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("tournament_group",'id',arguments.filter['id'])>
					</cfif>
				</cfif>
			
				<cfif StructKeyExists(arguments.filter,'season_id')>
					
							<cfset bFilterColumn = false>
							<cfif isNumeric(arguments.filter['season_id'])>
								<cfset bFilterColumn = true>
							</cfif>
						
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("tournament_group",'season_id',arguments.filter['season_id'])>
					</cfif>
				</cfif>
				...
				<cfif StructKeyExists(arguments.filter,'name')>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("tournament_group",'name',arguments.filter['name'])>
					</cfif>
				</cfif>
				...
				<cfif StructKeyExists(arguments.filter,'description')>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("tournament_group",'description',arguments.filter['description'])>
					</cfif>
				</cfif>
			
				<cfif StructKeyExists(arguments.filter,'maxsignups')>
					
							<cfset bFilterColumn = false>
							<cfif isNumeric(arguments.filter['maxsignups'])>
								<cfset bFilterColumn = true>
							</cfif>
						
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("tournament_group",'maxsignups',arguments.filter['maxsignups'])>
					</cfif>
				</cfif>
		...
		<cfset QueryRecordset.returnObjectFields("tournament_group","id,season_id,name,maxsignups")>
	...
	<cffunction name="getOptions" access="remote" output="false" returntype="query" hint="I get the option values for this table.">
		...
		<cfif len(variables.sOptionNameCode)>
			<cfset QueryRecordset.setFieldExpression("tournament_group",variables.sOptionNameField,variables.sOptionNameCode)>
		</cfif>
		<cfset QueryRecordset.setFieldAlias("tournament_group","id","optionvalue")>
		<cfset QueryRecordset.setFieldAlias("tournament_group",variables.sOptionNameField,"optionname")>
		<cfset QueryRecordset.returnObjectFields("tournament_group","optionvalue,optionname")>
	...
	<cffunction name="getRecordCount" output="false" returntype="numeric" hint="I get the number of records in the table.">
		...
		<cfset QueryCount.returnObjectFields("tournament_group","id") />
		<cfset QueryCount.setFieldExpression("tournament_group","id","COUNT(*)","CF_SQL_INTEGER") />
