<cfset aErrors = ArrayNew(1)>
	...
	<!--- snippet 'modules/tournament/controller/form/snippets/act_action_save_prevalidation_tournament_type_se_result.cfm' --->
	...
	<!--- snippet 'modules/tournament/controller/form/snippets/act_action_save_postvalidation_tournament_type_se_result.cfm' --->
	
	<!--- /snippet --->
	<cfparam name="attributes.tournament_type_se_result_id" default="0">
	...
	<cfif variables.mode EQ 'insert'>
		...
		<cfset otournament_type_se_result.setmatchid(attributes.matchid)>
		<cfset otournament_type_se_result.setteam1_result(attributes.team1_result)>
		<cfset otournament_type_se_result.setteam2_result(attributes.team2_result)>
	<cfelse>
	...
	<cfset otournament_type_se_result.validate()>
	<cfif otournament_type_se_result.hasErrors()>
		<cfset bHasErrors = true>
	</cfif>
	
	
	<cfif NOT bHasErrors>
		<cfset otournament_type_se_result.save()>
	...
	<cfelse>
		...
		<cfinclude template="act_form_loadrelated_tournament_type_se_result.cfm">
		
		<!--- snippet 'modules/tournament/controller/form/snippets/act_form_loadrelated_custom_tournament_type_se_result.cfm' --->
		
		<!--- /snippet --->
		<cfset aReactorErrors = otournament_type_se_result._getErrorCollection().getErrors()>
		<cfloop from="1" to="#ArrayLen(aReactorErrors)#" index="idx">
			<cfset ArrayAppend(aErrors,aReactorErrors[idx])>
		</cfloop>
		<cfset aReactorTranslatedErrors = otournament_type_se_result._getErrorCollection().getTranslatedErrors()>
		...
		<cfsavecontent variable="request.page.pageContent">
			...
			<cfinclude template="../../view/form/dsp_form_tournament_type_se_result.cfm">
		</cfsavecontent>
		<cfoutput>#request.page.pageContent#</cfoutput>
	</cfif>
