<cfcomponent displayname="news_trackbackGateway.cfc" extends="reactor.project.lanshock.Gateway.news_trackbackGateway">
...
<cfset variables.sOptionNameField = "entry_id">
	...
	<cffunction name="getRecords" access="public" hint="I return all matching rows from the table." output="false" returntype="query">
		...
		<cfif NOT StructIsEmpty(arguments.stFilter)>
			...
			<cfif StructKeyExists(arguments.stFilter,'stJoins')>
				<cfset Query.returnObjectFields("news_trackback","id,entry_id,blog_name,title,text,date,url")>
				
				<cfloop collection="#arguments.stFilter.stJoins#" item="idx">
					<cfset idx = lCase(idx)>
					<cfset Query.leftJoin("news_trackback",idx,idx)>
			...
			<cfif StructKeyExists(arguments.stFilter,'stFields')>
				<cfloop collection="#arguments.stFilter.stFields#" item="idx">
					...
					<cfelse>
						<cfset sCurrentTable = "news_trackback">
			...
			<cfif StructKeyExists(arguments.stFilter,'lSortFields')>
				<cfloop list="#arguments.stFilter.lSortFields#" index="idx">
					...
					<cfif ListLen(idx,'|') EQ 2>
						<cfset idx = ListPrepend(idx,"news_trackback",'|')>
					</cfif>
	...
	<cffunction name="getNWithJoin" output="false" returntype="query" hint="I get the selected N records, with an outer join to any parent tables.">
		...
		<cfif arguments.startrow GT 1>
			...
			<cfset QuerySkip.returnObjectFields("news_trackback","id") />
			<cfset QuerySkip.setMaxrows(arguments.startrow) />
			<cfset qSkip = getByQuery(QuerySkip) />
			<cfset where.isNotIn("news_trackback","id",valuelist(qSkip.id)) />
		</cfif>
		...
		<cfset QueryRecordset.returnObjectFields("news_trackback","id,entry_id,blog_name,title,text,date,url")>
	...
	<cffunction name="getOptions" access="remote" output="false" returntype="query" hint="I get the option values for this table.">
		...
		<cfif len(variables.sOptionNameCode)>
			<cfset QueryRecordset.setFieldExpression("news_trackback",variables.sOptionNameField,variables.sOptionNameCode)>
		</cfif>
		<cfset QueryRecordset.setFieldAlias("news_trackback","id","optionvalue")>
		<cfset QueryRecordset.setFieldAlias("news_trackback",variables.sOptionNameField,"optionname")>
		<cfset QueryRecordset.returnObjectFields("news_trackback","optionvalue,optionname")>
	...
	<cffunction name="getRecordCount" output="false" returntype="numeric" hint="I get the number of records in the table.">
		...
		<cfset QueryCount.returnObjectFields("news_trackback","id") />
		<cfset QueryCount.setFieldExpression("news_trackback","id","COUNT(*)","CF_SQL_INTEGER") />
