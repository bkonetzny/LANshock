<cfcomponent displayname="core_security_users_roles_relGateway.cfc" extends="reactor.project.lanshock.Gateway.core_security_users_roles_relGateway">
<!--- Start of gatewayCustom code generated by fusebox scaffolder, it will be replaced if fusebox scaffolder is rerun. --->
<cfset variables.sOptionNameField = "role_id">
	...
	<cffunction name="getRecords" access="public" hint="I return all matching rows from the table." output="false" returntype="query">
		...
		<cfif NOT StructIsEmpty(arguments.stFilter)>
			...
			<cfif StructKeyExists(arguments.stFilter,'stJoins')>
				<cfset Query.returnObjectFields("core_security_users_roles_rel","user_id,role_id")>
				
				<cfloop collection="#arguments.stFilter.stJoins#" item="idx">
					<cfset idx = lCase(idx)>
					<cfset Query.leftJoin("core_security_users_roles_rel",idx,idx)>
			...
			<cfif StructKeyExists(arguments.stFilter,'stFields')>
				<cfloop collection="#arguments.stFilter.stFields#" item="idx">
					...
					<cfelse>
						<cfset sCurrentTable = "core_security_users_roles_rel">
			...
			<cfif StructKeyExists(arguments.stFilter,'lSortFields')>
				<cfloop list="#arguments.stFilter.lSortFields#" index="idx">
					...
					<cfif ListLen(idx,'|') EQ 2>
						<cfset idx = ListPrepend(idx,"core_security_users_roles_rel",'|')>
					</cfif>
	...
	<cffunction name="getNWithJoin" output="false" returntype="query" hint="I get the selected N records, with an outer join to any parent tables.">
		...
		<cfif arguments.startrow GT 1>
			...
			<cfset QuerySkip.returnObjectFields("core_security_users_roles_rel","role_id") />
			<cfset QuerySkip.setMaxrows(arguments.startrow) />
			<cfset qSkip = getByQuery(QuerySkip) />
			<cfset where.isNotIn("core_security_users_roles_rel","role_id",valuelist(qSkip.role_id)) />
		</cfif>
		...
		<cfif isStruct(arguments.filter)>
			<cfset QueryRecordset.getWhere().setMode("OR")>
				...
				<cfif StructKeyExists(arguments.filter,'user_id')>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("core_security_users_roles_rel",'user_id',arguments.filter['user_id'])>
					</cfif>
				</cfif>
				...
				<cfif StructKeyExists(arguments.filter,'role_id')>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("core_security_users_roles_rel",'role_id',arguments.filter['role_id'])>
					</cfif>
				</cfif>
		...
		<cfset QueryRecordset.returnObjectFields("core_security_users_roles_rel","user_id,role_id")>
	...
	<cffunction name="getOptions" access="remote" output="false" returntype="query" hint="I get the option values for this table.">
		...
		<cfif len(variables.sOptionNameCode)>
			<cfset QueryRecordset.setFieldExpression("core_security_users_roles_rel",variables.sOptionNameField,variables.sOptionNameCode)>
		</cfif>
		<cfset QueryRecordset.setFieldAlias("core_security_users_roles_rel","role_id","optionvalue")>
		<cfset QueryRecordset.setFieldAlias("core_security_users_roles_rel",variables.sOptionNameField,"optionname")>
		<cfset QueryRecordset.returnObjectFields("core_security_users_roles_rel","optionvalue,optionname")>
	...
	<cffunction name="getRecordCount" output="false" returntype="numeric" hint="I get the number of records in the table.">
		...
		<cfset QueryCount.returnObjectFields("core_security_users_roles_rel","role_id") />
		<cfset QueryCount.setFieldExpression("core_security_users_roles_rel","role_id","COUNT(*)","CF_SQL_INTEGER") />
		<cfset qCount = getByQuery(QueryCount) />
		
		<!--- Return the recordcount --->
		<cfreturn qCount.role_id />
	</cffunction>
