<cfsilent>
<!--- -->
<fusedoc fuse="$RCSfile: dsp_list_core_logs.cfm,v $" language="ColdFusion 7.01" version="2.0"  >
	<responsibilities>
		This page displays a listing of the core_logs records from a recordset. 
		Each record has a link for viewing, deleting, and editing the selected core_logs record.
	</responsibilities>
	<properties>
		...
		<property name="lastupdated" value="$Date: 2008-05-31 13:51:47 +0200 (Sa, 31 Mai 2008) 2008/05/31 14:33:51 $" />
	...
	<io>
		<in>
			...
			<number name="_totalRowCount" precision="integer" scope="variables" comments="Count of rows in the core_logs table." />
			<string name="_listSortByFieldList" default="">
			
			<recordset name="qcore_logs" primaryKeys="timestamp" scope="variables" comments="Recordset containing core_logs records " >
			...
			<list name="fieldlist" scope="variables" optional="Yes" 
				default="id,logname,level,data,timestamp" 
...
<script type="text/javascript">
	Ext.onReady(function(){
        ...
        var action = new Ext.ux.grid.RowActions({
			header:'#jsStringFormat(request.content.core_logs_grid_header__rowactions)#',
			actions:[{
				iconCls:'icon-edit-record',
				tooltip:'#jsStringFormat(request.content.core_logs_grid_row_edit)#'
			},{
				iconCls:'icon-delete-record',
				tooltip:'#jsStringFormat(request.content.core_logs_grid_row_delete)#'
			}]
		});
		...
		action.on({
			action:function(grid, record, action, row, col) {
				if(action == 'icon-edit-record'){
					window.location.href='#myself##xfa.update#&timestamp=' + grid.getSelectionModel().getSelected().id;
				}
	    ...
	    var grid = new xg.GridPanel({
	        ...
	        cm: new xg.ColumnModel([
	        	sm,
	        	{id:'id',header:'#jsStringFormat(request.content.core_logs_grid_header_id)#',width:30,sortable:true,dataIndex:'id'},{header:'#jsStringFormat(request.content.core_logs_grid_header_logname)#',width:30,sortable:true,dataIndex:'logname'},{header:'#jsStringFormat(request.content.core_logs_grid_header_level)#',width:30,sortable:true,dataIndex:'level'},{header:'#jsStringFormat(request.content.core_logs_grid_header_data)#',width:30,sortable:true,dataIndex:'data'},{header:'#jsStringFormat(request.content.core_logs_grid_header_timestamp)#',width:30,sortable:true,dataIndex:'timestamp'},
	        	action
	        ]),
	        sm: sm,
	        ...
	        // inline toolbars
	        tbar:[{
	            text:'#jsStringFormat(request.content.core_logs_grid_global_add)#',
	            tooltip:'#jsStringFormat(request.content.core_logs_grid_global_add)#',
	            iconCls:'add',
	            handler:function(){window.location.href='#myself##xfa.add#';}
	        },'-',{
	            text:'#jsStringFormat(request.content.core_logs_grid_global_delete)#',
	            tooltip:'#jsStringFormat(request.content.core_logs_grid_global_delete)#',
	            iconCls:'remove',
	            handler: doDel
	        }],
	        ...
	        height:533,
	        frame:false,
	        renderTo: Ext.get('grid_core_logs'),
	    ...
	    function doDel(){
	        ...
	        if(m.length > 0) Ext.MessageBox.confirm('Message','#jsStringFormat(request.content.core_logs_grid_global_delete_warning_confirm)#',doDel2);
	        else Ext.MessageBox.alert('Message','#jsStringFormat(request.content.core_logs_grid_global_delete_warning_select)#');
	    }     
	    ...
	    function doDel2(btn){
	       if(btn == 'yes') {	
		        ...
		        for(var i = 0, len = m.length; i < len; i++){        		
					var ss = "{\"timestamp\":\"" + m[i].get("timestamp") + "\"}";
