...
<cfparam name="variables.fieldlist" default="id,name,email,pwd,firstname,lastname,gender,status,signature,homepage,internal_note,dt_birthdate,dt_lastlogin,dt_registered,logincount,language,geo_latlong,country,city,street,zip,reset_password_key,openid_url,data_access">
</cfsilent>
<cfoutput>
...
<script type="text/javascript">
	Ext.onReady(function(){
		...
		var grid = new xg.GridPanel({
	        ...
	        viewConfig: {forceFit: true}, renderTo: Ext.get('grid_user'),
	        bbar: new Ext.PagingToolbar({pageSize: 20, store: ds, displayInfo: true}),
			cm: new xg.ColumnModel([
	        	sm,
	        	{id:'id',header:'#jsStringFormat(request.content.user_grid_header_id)#',sortable:true,dataIndex:'id',width:30},{header:'#jsStringFormat(request.content.user_grid_header_name)#',sortable:true,dataIndex:'name',width:30},{header:'#jsStringFormat(request.content.user_grid_header_email)#',sortable:true,dataIndex:'email',width:30},{header:'#jsStringFormat(request.content.user_grid_header_firstname)#',sortable:true,dataIndex:'firstname',width:30},{header:'#jsStringFormat(request.content.user_grid_header_lastname)#',sortable:true,dataIndex:'lastname',width:30},
	        	action
	        ]),
