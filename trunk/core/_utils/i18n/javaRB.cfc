...
<cfscript>
	...
	var thisRBfile=trim(thisDir & listFirst(thisFile,".") & "_"& arguments.rbLocale & "." & listLast(thisFile,"."));
	if (NOT fileExists(thisRBfile)) //try just the language
		thisRBfile=trim(thisDir & listFirst(thisFile,".") & "_"& thisLang & "." & listLast(thisFile,"."));
	if (NOT fileExists(thisRBfile))// still nothing? strip thisRBfile back to base rb
		thisRBFile=trim(arguments.rbFile);
