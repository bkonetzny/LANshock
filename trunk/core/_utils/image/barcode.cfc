...
<cfcomponent>
	...
	<cffunction name="outputBarcode">
		...
		<cfset createBarcode(code=arguments.code,type=arguments.type,format=arguments.format,mode='output',width=arguments.width,height=arguments.height)>
	</cffunction>
	...
	<cffunction name="saveBarcode">
		...
		<cfset createBarcode(code=arguments.code,type=arguments.type,format=arguments.format,mode='save',width=arguments.width,height=arguments.height,saveToFile=arguments.saveToFile)>
	</cffunction>
	...
	<cffunction name="createBarcode" returntype="any" access="private" output="true">
		...
		<cfset aJarPath[1] = expandPath("./core/_utils/java/barbecue/barbecue-1.5-beta1.jar")>
		...
		<cfif arguments.width GT 0>
			<cfset barcode.setBarWidth(arguments.width)>
		</cfif>
		<cfif arguments.height GT 0>
			<cfset barcode.setBarHeight(arguments.height)>
		</cfif>
		...
		<cfif arguments.mode EQ 'output'>
			<cfset getPageContext().getOut().clearBuffer()><cfcontent type="image/jpeg" variable="#outStream.toByteArray()#"><cfset outStream.close()>
		<cfelseif len(arguments.saveToFile)>
