		...
		if (structKeyExists(fb_.verbInfo.attributes,"url")) {
			if (structKeyExists(fb_.verbInfo.attributes,"xfa")) {
				fb_throw("fusebox.badGrammar.requiredAttributeMissing",
							...
							"Either the attribute 'url' or 'xfa' is required, for a 'lanshock:relocate' verb in fuseaction #fb_.verbInfo.circuit#.#fb_.verbInfo.fuseaction#.");
			} else {
			...
			if (structKeyExists(fb_.verbInfo.attributes,"xfa")) {
				...
				fb_.theModule = "##xfa." & fb_.verbInfo.attributes.xfa & "##";
			} else {
				fb_throw("fusebox.badGrammar.requiredAttributeMissing",
							"Required attribute is missing",
							"Either the attribute 'url' or 'xfa' is required, for a 'lanshock:relocate' verb in fuseaction #fb_.verbInfo.circuit#.#fb_.verbInfo.fuseaction#.");
		...
		if (structKeyExists(fb_.verbInfo.attributes,"type")) {
			...
			if (listFind("server,client,moved,javascript",fb_.verbInfo.attributes.type) eq 0) {
				fb_throw("fusebox.badGrammar.invalidAttributeValue",
							...
							"The attribute 'type' must either be ""server"", ""client"", ""moved"" or ""javascript"", for a 'lanshock:relocate' verb in fuseaction #fb_.verbInfo.circuit#.#fb_.verbInfo.fuseaction#.");
			}
		} else {
		...
		if (fb_.verbInfo.action.getCircuit().getApplication().strictMode) {
			if (structCount(fb_.verbInfo.attributes) neq 2) {
				fb_throw("fusebox.badGrammar.unexpectedAttributes",
							...
							"Unexpected attributes were found in a 'lanshock:relocate' verb in fuseaction #fb_.verbInfo.circuit#.#fb_.verbInfo.fuseaction#.");
			}
		}
