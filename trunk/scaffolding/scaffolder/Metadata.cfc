...
--->
	...
	<cffunction name="build" returntype="void" output="Yes" 
		...
		<cfargument name="sModule" type="string" required="No" default="mymodule" 
					hint="I am the name of the module to generate"/>
		...
		<cfset setModule(arguments.sModule)>
		...
		<cfloop index="i" from="1" to="#ArrayLen(aTemplateFiles)#">
			<cfif NOT aTemplateFiles[i].perObject>
				<cfset thisOutputPath = "#aTemplateFiles[i].MVCpath##aTemplateFiles[i].outputFile#.#aTemplateFiles[i].suffix#">
			...
			<cfloop index="i" from="1" to="#ArrayLen(aTemplateFiles)#">
				<cfif aTemplateFiles[i].perObject>
					...
					<cfif aTemplateFiles[i].suffix IS "cfc" OR aTemplateFiles[i].suffix IS "as">
						<cfset thisOutputPath = "#aTemplateFiles[i].MVCpath##aliasInName##aTemplateFiles[i].outputFile#.#aTemplateFiles[i].suffix#">
						<cfset thisOutputPath = replaceNoCase(thisOutputPath,'$tablename$',arguments.sModule,'ALL')>
					<cfelse>
						<cfset thisOutputPath = "#aTemplateFiles[i].MVCpath##aTemplateFiles[i].outputFile##aliasInName#.#aTemplateFiles[i].suffix#">
						<cfset thisOutputPath = replaceNoCase(thisOutputPath,'$tablename$',arguments.sModule,'ALL')>
					</cfif>
					<cfset cftemplate.generateScript("#arguments.template#/#aTemplateFiles[i].templateFile#.#aTemplateFiles[i].suffix#",this,thisOutputPath,aTemplateFiles[i].inPlace,aTemplateFiles[i].overwrite)>
				</cfif>
...
<!--- *** 							 *** --->
	...
	<cffunction name="GetAllDatasources" returntype="struct" output="No" 
		...
		<cfset var stDatasourcesRailo = StructNew()>
		<cfset var sConnection = ''>
		<cfset var idx = 0>
		...
		<cfif FindNoCase(server.coldfusion.productname,'Railo')>
			<cfloop collection="#stDatasources#" item="idx">
				<cfset stDatasourcesRailo[idx] = StructNew()>
				<cfset stDatasourcesRailo[idx].name = idx>
				<cfset stDatasourcesRailo[idx].buffer = 1024>
				<cfset stDatasourcesRailo[idx].urlmap.database = idx>
				<cfset sConnection = stDatasources[idx].getConnection().toString()>
				<cfif FindNoCase('mysql',sConnection)>
					<cfset stDatasourcesRailo[idx].driver = 'mysql'>
				<cfelseif FindNoCase('mssql',sConnection)>
					<cfset stDatasourcesRailo[idx].driver = 'mssql'>
				</cfif>
			</cfloop>
		
			<cfreturn stDatasourcesRailo>
		<cfelse>
...
<scaffolding>
	...
	<cffunction name="getFieldsMySQL" returntype="query" output="No" 
		...
		<cfquery name="qFields" datasource="#arguments.datasource#" username="#arguments.username#" password="#arguments.password#">
			SELECT	column_name,
		...
		<cfloop query="qFields">
			<cfif NOT len(qFields.length)>
				<cfif qFields.type_name EQ 'datetime'>
					<cfset QuerySetCell(qFields,'length',19,qFields.currentrow)>
				<cfelse>
					<cfset QuerySetCell(qFields,'length',ReReplaceNoCase(qFields.column_type,'[a-z()]','','ALL'),qFields.currentrow)>
				</cfif>
			</cfif>
			<cfif len(qFields.column_key)>
				<cfset QuerySetCell(qFields,'key_seq',1,qFields.currentrow)>
			</cfif>
		</cfloop>
	...
	<cffunction name="getParentRelationshipsMySQL" returntype="query" output="No" 
				hint="I return a query containing the parent relationships for the selected table.">
		<cfargument name="datasource" required="Yes" type="string" hint="I am a datasource name.">
		<cfargument name="tableName" type="string" required="Yes">
		<cfargument name="username" required="No" default="" type="string" hint="I am the username.">
		...
		<cfset var qFKFields=0>
		
		<cfset qFKFields=QueryNew('no_column')>
...
<!--- *** 													  **** --->
	...
	<cffunction name="setModule" returntype="void" output="No" >
		...
		<cfset variables.sModule = arguments.sModule>
	</cffunction>
	<cffunction name="getModule" returntype="string">
		<cfreturn variables.sModule>
	</cffunction>
