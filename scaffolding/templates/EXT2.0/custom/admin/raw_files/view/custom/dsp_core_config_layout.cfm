...
<!--
	function setPreviewImage(template){
		document.getElementById("imgPreviewImage").src = "#application.lanshock.oRuntime.getEnvironment().sWebPath#templates/" + template +"/_preview.jpg";
	}
...
<table class="vlist">
	<tr>
		...
		<td><select name="template" onChange="setPreviewImage(escape(getSelectedValue(document.formTemplate.template)))">
			...
			<img id="imgPreviewImage" style="border: 1px dotted gray;" vspace="2" hspace="2" src="#application.lanshock.oRuntime.getEnvironment().sWebPath#templates/#UrlEncodedFormat(application.lanshock.settings.layout.template)#/_preview.jpg"></td>
	</tr>
	<tr>
