...
<<cfloop from="1" to="$$ArrayLen(aJoinedFields)$$" index="i">>
<cfset stJoinedFields["$$aJoinedFields[i].alias$$"] = "$$aJoinedFields[i].table$$">><</cfloop>>
...
<cfif isDefined("o$$objectname$$")>
	...
	<cfloop list="#fieldlist#" index="thisField">
		<cfif ListFindNoCase(_joinedFieldlist,thisField)>
			<cfset setvariable("variables.#thisField#", evaluate("o$$objectname$$.get" & stJoinedFields[thisField] & "().get" & thisField & "()"))>
		<cfelse>
			...
			<cfcase value="$$aFields[i].alias$$">
				<tr>
				  ...
				  <cfelse>
					...
					<td>
						#$$Format("variables.$$aFields[i].alias$$","$$aFields[i].format$$")$$#
					</td>
				</tr>
			</cfcase><</cfloop>>
			<<cfloop from="1" to="$$ArrayLen(aJoinedFields)$$" index="i">><<cfif NOT ListFindNoCase(lFields,aJoinedFields[i].alias)>>
			<cfcase value="$$aJoinedFields[i].alias$$">
				<tr>
				  ...
				  <cfelse>
					...
					<td>
						#$$Format("variables.$$aJoinedFields[i].alias$$","$$aJoinedFields[i].format$$")$$#
					</td>
				</tr>
			</cfcase><</cfif>><</cfloop>>
		  </cfswitch>
