...
<cfcomponent>
	...
	<cffunction name="getNews" access="remote" returntype="query" output="false">
		...
		<cfif arguments.category_id EQ 0>
			<cfquery datasource="#request.lanshock.environment.datasource#" name="qNews">
				...
				<cfif arguments.user_id NEQ 0>
					AND author = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.user_id#">
				</cfif>
				ORDER BY date DESC
		...
		<cfelse>
			<cfquery datasource="#request.lanshock.environment.datasource#" name="qNews">
				SELECT *
				FROM news_entry e, news_entry_category ec
				WHERE e.id = ec.entry_id
				AND ec.category_id = <cfqueryparam cfsqltype="cf_sql_integer" value="#arguments.category_id#">
				<cfif NOT arguments.showfuture>
					AND date <= #now()#
				</cfif>
				<cfif arguments.user_id NEQ 0>
