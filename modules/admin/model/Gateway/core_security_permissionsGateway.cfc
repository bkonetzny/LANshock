<cfcomponent displayname="core_security_permissionsGateway.cfc" extends="reactor.project.lanshock.Gateway.core_security_permissionsGateway">
...
<cfset variables.sOptionNameField = "name">
	...
	<cffunction name="getRecords" access="public" hint="I return all matching rows from the table." output="false" returntype="query">
		...
		<cfif NOT StructIsEmpty(arguments.stFilter)>
			...
			<cfif StructKeyExists(arguments.stFilter,'stJoins')>
				<cfset Query.returnObjectFields("core_security_permissions","id,name,module")>
				
				<cfloop collection="#arguments.stFilter.stJoins#" item="idx">
					<cfset idx = lCase(idx)>
					<cfset Query.leftJoin("core_security_permissions",idx,idx)>
			...
			<cfif StructKeyExists(arguments.stFilter,'stFields')>
				<cfloop collection="#arguments.stFilter.stFields#" item="idx">
					...
					<cfelse>
						<cfset sCurrentTable = "core_security_permissions">
			...
			<cfif StructKeyExists(arguments.stFilter,'lSortFields')>
				<cfloop list="#arguments.stFilter.lSortFields#" index="idx">
					...
					<cfif ListLen(idx,'|') EQ 2>
						<cfset idx = ListPrepend(idx,"core_security_permissions",'|')>
					</cfif>
	...
	<cffunction name="getNWithJoin" output="false" returntype="query" hint="I get the selected N records, with an outer join to any parent tables.">
		...
		<cfif arguments.startrow GT 1>
			...
			<cfset QuerySkip.returnObjectFields("core_security_permissions","id") />
			<cfset QuerySkip.setMaxrows(arguments.startrow) />
			<cfset qSkip = getByQuery(QuerySkip) />
			<cfset where.isNotIn("core_security_permissions","id",valuelist(qSkip.id)) />
		</cfif>
		...
		<cfif isStruct(arguments.filter)>
			<cfset QueryRecordset.getWhere().setMode("OR")>
				...
				<cfif StructKeyExists(arguments.filter,'id')>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("core_security_permissions",'id',arguments.filter['id'])>
					</cfif>
				</cfif>
				...
				<cfif StructKeyExists(arguments.filter,'name')>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("core_security_permissions",'name',arguments.filter['name'])>
					</cfif>
				</cfif>
				...
				<cfif StructKeyExists(arguments.filter,'module')>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("core_security_permissions",'module',arguments.filter['module'])>
					</cfif>
				</cfif>
		...
		<cfset QueryRecordset.returnObjectFields("core_security_permissions","id,name,module")>
	...
	<cffunction name="getOptions" access="remote" output="false" returntype="query" hint="I get the option values for this table.">
		...
		<cfif len(variables.sOptionNameCode)>
			<cfset QueryRecordset.setFieldExpression("core_security_permissions",variables.sOptionNameField,variables.sOptionNameCode)>
		</cfif>
		<cfset QueryRecordset.setFieldAlias("core_security_permissions","id","optionvalue")>
		<cfset QueryRecordset.setFieldAlias("core_security_permissions",variables.sOptionNameField,"optionname")>
		<cfset QueryRecordset.returnObjectFields("core_security_permissions","optionvalue,optionname")>
	...
	<cffunction name="getRecordCount" output="false" returntype="numeric" hint="I get the number of records in the table.">
		...
		<cfset QueryCount.returnObjectFields("core_security_permissions","id") />
		<cfset QueryCount.setFieldExpression("core_security_permissions","id","COUNT(*)","CF_SQL_INTEGER") />
