<cfcomponent displayname="core_security_roles_permissions_relGateway.cfc" extends="reactor.project.lanshock.Gateway.core_security_roles_permissions_relGateway">
...
<cfset variables.sOptionNameField = "role_id">
	...
	<cfset variables.sOptionNameField = "role_id">
	<cfset variables.sOptionNameCode = "concat(""role_id: "",role_id,"" | permission_id: "",permission_id)">
		...
		<cfif NOT StructIsEmpty(arguments.stFilter)>
			...
			<cfif StructKeyExists(arguments.stFilter,'stJoins')>
				...
				<cfloop collection="#arguments.stFilter.stJoins#" item="idx">
					<cfset idx = lCase(idx)>
					<cfset Query.leftJoin("core_security_roles_permissions_rel",idx,idx)>
			...
			<cfif StructKeyExists(arguments.stFilter,'stFields')>
				<cfloop collection="#arguments.stFilter.stFields#" item="idx">
					...
					<cfelse>
						<cfset sCurrentTable = "core_security_roles_permissions_rel">
			...
			<cfif StructKeyExists(arguments.stFilter,'lSortFields')>
				<cfloop list="#arguments.stFilter.lSortFields#" index="idx">
					...
					<cfif ListLen(idx,'|') EQ 2>
						<cfset idx = ListPrepend(idx,"core_security_roles_permissions_rel",'|')>
					</cfif>
	...
	<cffunction name="getNWithJoin" output="false" returntype="query" hint="I get the selected N records, with an outer join to any parent tables.">
		...
		<cfif arguments.startrow GT 1>
			...
			<cfset QuerySkip.returnObjectFields("core_security_roles_permissions_rel","id") />
			<cfset QuerySkip.setMaxrows(arguments.startrow) />
			<cfset qSkip = getByQuery(QuerySkip) />
			<cfset where.isNotIn("core_security_roles_permissions_rel","id",valuelist(qSkip.id)) />
		</cfif>
	...
	<cffunction name="getOptions" access="remote" output="false" returntype="query" hint="I get the option values for this table.">
		...
		<cfif len(variables.sOptionNameCode)>
			<cfset QueryRecordset.setFieldExpression("core_security_roles_permissions_rel",variables.sOptionNameField,variables.sOptionNameCode)>
		</cfif>
		<cfset QueryRecordset.setFieldAlias("core_security_roles_permissions_rel","id","optionvalue")>
		<cfset QueryRecordset.setFieldAlias("core_security_roles_permissions_rel",variables.sOptionNameField,"optionname")>
		<cfset QueryRecordset.returnObjectFields("core_security_roles_permissions_rel","optionvalue,optionname")>
	...
	<cffunction name="getRecordCount" output="false" returntype="numeric" hint="I get the number of records in the table.">
		...
		<cfset QueryCount.returnObjectFields("core_security_roles_permissions_rel","id") />
		<cfset QueryCount.setFieldExpression("core_security_roles_permissions_rel","id","COUNT(*)","CF_SQL_INTEGER") />
