<cfcomponent displayname="core_cronGateway.cfc" extends="reactor.project.lanshock.Gateway.core_cronGateway">
<!--- Start of gatewayCustom code generated by fusebox scaffolder, it will be replaced if fusebox scaffolder is rerun. --->
<cfset variables.sOptionNameField = "active">
	...
	<cffunction name="getRecords" access="public" hint="I return all matching rows from the table." output="false" returntype="query">
		...
		<cfif NOT StructIsEmpty(arguments.stFilter)>
			...
			<cfif StructKeyExists(arguments.stFilter,'stJoins')>
				<cfset Query.returnObjectFields("core_cron","id,active,run,module,action,executions,lastrun_dt,lastrun_time,result")>
				
				<cfloop collection="#arguments.stFilter.stJoins#" item="idx">
					<cfset idx = lCase(idx)>
					<cfset Query.leftJoin("core_cron",idx,idx)>
			...
			<cfif StructKeyExists(arguments.stFilter,'lSortFields')>
				<cfloop list="#arguments.stFilter.lSortFields#" index="idx">
					...
					<cfif ListLen(idx,'|') EQ 2>
						<cfset idx = ListPrepend(idx,"core_cron",'|')>
					</cfif>
	...
	<cffunction name="getNWithJoin" output="false" returntype="query" hint="I get the selected N records, with an outer join to any parent tables.">
		...
		<cfif arguments.startrow GT 1>
			...
			<cfset QuerySkip.returnObjectFields("core_cron","id") />
			<cfset QuerySkip.setMaxrows(arguments.startrow) />
			<cfset qSkip = getByQuery(QuerySkip) />
			<cfset where.isNotIn("core_cron","id",valuelist(qSkip.id)) />
		</cfif>
		...
		<cfif isStruct(arguments.filter)>
			<cfset QueryRecordset.getWhere().setMode("OR")>
				...
				<cfif StructKeyExists(arguments.filter,'id')>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("core_cron",'id',arguments.filter['id'])>
					</cfif>
				</cfif>
				...
				<cfif StructKeyExists(arguments.filter,'active')>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("core_cron",'active',arguments.filter['active'])>
					</cfif>
				</cfif>
			
				<cfif StructKeyExists(arguments.filter,'run')>
					
							<cfset bFilterColumn = true>
						
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("core_cron",'run',arguments.filter['run'])>
					</cfif>
				</cfif>
				...
				<cfif StructKeyExists(arguments.filter,'module')>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("core_cron",'module',arguments.filter['module'])>
					</cfif>
				</cfif>
				...
				<cfif StructKeyExists(arguments.filter,'action')>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("core_cron",'action',arguments.filter['action'])>
					</cfif>
				</cfif>
			
				<cfif StructKeyExists(arguments.filter,'executions')>
					
							<cfset bFilterColumn = false>
							<cfif isNumeric(arguments.filter['executions'])>
								<cfset bFilterColumn = true>
							</cfif>
						
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("core_cron",'executions',arguments.filter['executions'])>
					</cfif>
				</cfif>
			
				<cfif StructKeyExists(arguments.filter,'lastrun_dt')>
					
							<cfset bFilterColumn = false>
							<cfif LsIsDate(arguments.filter['lastrun_dt'])>
								<cfset bFilterColumn = true>
							</cfif>
						
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("core_cron",'lastrun_dt',arguments.filter['lastrun_dt'])>
					</cfif>
				</cfif>
			
				<cfif StructKeyExists(arguments.filter,'lastrun_time')>
					
							<cfset bFilterColumn = false>
							<cfif isNumeric(arguments.filter['lastrun_time'])>
								<cfset bFilterColumn = true>
							</cfif>
						
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("core_cron",'lastrun_time',arguments.filter['lastrun_time'])>
					</cfif>
				</cfif>
				...
				<cfif StructKeyExists(arguments.filter,'result')>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("core_cron",'result',arguments.filter['result'])>
					</cfif>
				</cfif>
		...
		<cfset QueryRecordset.returnObjectFields("core_cron","id,active,run,module,action,lastrun_dt,lastrun_time")>
	...
	<cffunction name="getOptions" access="remote" output="false" returntype="query" hint="I get the option values for this table.">
		...
		<cfif len(variables.sOptionNameCode)>
			<cfset QueryRecordset.setFieldExpression("core_cron",variables.sOptionNameField,variables.sOptionNameCode)>
		</cfif>
		<cfset QueryRecordset.setFieldAlias("core_cron","id","optionvalue")>
		<cfset QueryRecordset.setFieldAlias("core_cron",variables.sOptionNameField,"optionname")>
		<cfset QueryRecordset.returnObjectFields("core_cron","optionvalue,optionname")>
	...
	<cffunction name="getRecordCount" output="false" returntype="numeric" hint="I get the number of records in the table.">
		...
		<cfset QueryCount.returnObjectFields("core_cron","id") />
		<cfset QueryCount.setFieldExpression("core_cron","id","COUNT(*)","CF_SQL_INTEGER") />
