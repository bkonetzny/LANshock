<cfsilent>
<!--- -->
<fusedoc fuse="$RCSfile: dsp_display_core_navigation.cfm,v $" language="ColdFusion 7.01" version="2.0">
	<responsibilities>
	...
	<io>
		<in>
			...
			<object name="ocore_navigation" comments="The record object to be displayed." optional="Yes" />
...
<cfset editParams = appendParam(pageParams,"action",attributes.action)>
...
<cfif isDefined("ocore_navigation")>
	...
	<cfloop list="#fieldlist#" index="thisField">
		<cfif ListFindNoCase(_joinedFieldlist,thisField)>
			<cfset setvariable("variables.#thisField#", evaluate("ocore_navigation.get" & stJoinedFields[thisField] & "().get" & thisField & "()"))>
		<cfelse>
			<cfset setvariable("variables.#thisField#", evaluate("ocore_navigation.get" & thisField & "()"))>
		</cfif>
	</cfloop>
<cfelseif isDefined("stcore_navigation")>
	<!--- Copy variables from the structure. --->
	<cfloop list="#fieldlist#" index="thisField">
		<cfset setvariable("variables.#thisField#", stcore_navigation[thisField])>
	</cfloop>
<cfelseif isDefined("qcore_navigation")>
	<cfparam name="_recordNumber" default="1">
	<!--- Copy variables from the query. --->
	<cfloop list="#fieldlist#" index="thisField">
		<cfset setvariable("variables.#thisField#", qcore_navigation[thisField][_recordNumber])>
	</cfloop>
<cfelse>
...
<!--- Start of the form --->
<cfoutput>
	<table width="590" border="0" cellpadding="2" cellspacing="2" summary="This table shows details of a single core_navigation record." class="">
			...
			<cfcase value="level">
				<tr>
				  ...
				  <cfelse>
					...
					<td>
						#NumberFormat(variables.level,"9")#
					</td>
				</tr>
			</cfcase>
			<cfcase value="sortorder">
				<tr>
				  <cfif ListFindNocase(highlightfields,'sortorder')>
					<th align="left" class="highlight">
				  <cfelse>
					<th align="left" class="standard">
				  </cfif>
					...
					<td>
						#NumberFormat(variables.sortorder,"9")#
					</td>
				</tr>
			</cfcase>
