...
<cfparam name="variables.fieldlist" default="name,version,date,folder">
</cfsilent>
<cfoutput>
...
<script type="text/javascript">
	Ext.onReady(function(){
		...
		action.on({
			action:function(grid, record, action, row, col) {
				if(action == 'icon-edit-record'){
					window.location.href='#application.lanshock.oHelper.buildUrl('#xfa.update#')#&folder=' + grid.getSelectionModel().getSelected().id;
				}
	    ...
	    var ds = new Ext.data.GroupingStore({
			...
			reader: new Ext.data.JsonReader({totalProperty: 'totalRecords', root: 'data', id: 'folder'},[
				{name:'name',mapping:'name'},{name:'version',mapping:'version'},{name:'date',mapping:'date'},{name:'folder',mapping:'folder'}
			]),
			sortInfo: {field:'folder',direction:'DESC'},
			remoteSort: true,
			autoLoad: false
		});
		var grid = new xg.GridPanel({
			id:'button-grid', ds: ds, sm: sm, height: 533, frame: false,
	        viewConfig: {forceFit: true}, renderTo: Ext.get('grid_core_modules'),
	        bbar: new Ext.PagingToolbar({pageSize: 20, store: ds, displayInfo: true}),
			cm: new xg.ColumnModel([
	        	sm,
	        	{id:'folder',header:'#jsStringFormat(request.content.core_modules_grid_header_name)#',sortable:true,dataIndex:'name',width:30},{header:'#jsStringFormat(request.content.core_modules_grid_header_version)#',sortable:true,dataIndex:'version',width:30},{header:'#jsStringFormat(request.content.core_modules_grid_header_date)#',sortable:true,dataIndex:'date',width:50,renderer:LANshock.Formatters.cfTimeStamp},{header:'#jsStringFormat(request.content.core_modules_grid_header_folder)#',sortable:true,dataIndex:'folder',width:30},
	        	action
	        ]),
