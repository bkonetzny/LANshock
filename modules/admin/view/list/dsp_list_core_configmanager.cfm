...
<script type="text/javascript">
	Ext.onReady(function(){
		...
		action.on({
			action:function(grid, record, action, row, col) {
				if(action == 'icon-edit-record'){
					window.location.href='#application.lanshock.oHelper.buildUrl('#xfa.update#')#&module=' + grid.getSelectionModel().getSelected().id;
				}
	    ...
	    var ds = new Ext.data.GroupingStore({
			...
			reader: new Ext.data.JsonReader({totalProperty: 'totalRecords', root: 'data', id: 'module'},[
				{name:'module',mapping:'module'},{name:'version',mapping:'version'},{name:'data',mapping:'data'},{name:'dtlastchanged',mapping:'dtlastchanged'}
			]),
			sortInfo: {field:'module',direction:'DESC'},
			remoteSort: true,
			autoLoad: false
		});
		var grid = new xg.GridPanel({
			id:'button-grid', ds: ds, sm: sm, height: 533, frame: false,
	        viewConfig: {forceFit: true}, renderTo: Ext.get('grid_core_configmanager'),
	        bbar: new Ext.PagingToolbar({pageSize: 20, store: ds, displayInfo: true}),
			cm: new xg.ColumnModel([
	        	sm,
	        	{id:'module',header:'#jsStringFormat(request.content.core_configmanager_grid_header_module)#',sortable:true,dataIndex:'module',width:30},{header:'#jsStringFormat(request.content.core_configmanager_grid_header_version)#',sortable:true,dataIndex:'version',width:30},{header:'#jsStringFormat(request.content.core_configmanager_grid_header_data)#',sortable:true,dataIndex:'data',width:30},{header:'#jsStringFormat(request.content.core_configmanager_grid_header_dtlastchanged)#',sortable:true,dataIndex:'dtlastchanged',width:50,renderer:LANshock.Formatters.cfTimeStamp},
	        	action
	        ]),
