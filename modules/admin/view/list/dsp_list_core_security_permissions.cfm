<cfsilent>
<!--- -->
<fusedoc fuse="$RCSfile: dsp_list_core_security_permissions.cfm,v $" language="ColdFusion 7.01" version="2.0"  >
	<responsibilities>
		This page displays a listing of the core_security_permissions records from a recordset. 
		Each record has a link for viewing, deleting, and editing the selected core_security_permissions record.
	</responsibilities>
	<properties>
		...
		<property name="lastupdated" value="$Date: 2008-05-31 13:51:47 +0200 (Sa, 31 Mai 2008) 2008/05/31 14:33:54 $" />
	...
	<io>
		<in>
			...
			<number name="_totalRowCount" precision="integer" scope="variables" comments="Count of rows in the core_security_permissions table." />
			<string name="_listSortByFieldList" default="">
			
			<recordset name="qcore_security_permissions" primaryKeys="id" scope="variables" comments="Recordset containing core_security_permissions records " >
...
<script type="text/javascript">
	Ext.onReady(function(){
        ...
        var action = new Ext.ux.grid.RowActions({
			header:'#jsStringFormat(request.content.core_security_permissions_grid_header__rowactions)#',
			actions:[{
				iconCls:'icon-edit-record',
				tooltip:'#jsStringFormat(request.content.core_security_permissions_grid_row_edit)#'
			},{
				iconCls:'icon-delete-record',
				tooltip:'#jsStringFormat(request.content.core_security_permissions_grid_row_delete)#'
			}]
		});
	    ...
	    var grid = new xg.GridPanel({
	        ...
	        cm: new xg.ColumnModel([
	        	sm,
	        	{id:'id',header:'#jsStringFormat(request.content.core_security_permissions_grid_header_id)#',width:30,sortable:true,dataIndex:'id'},{header:'#jsStringFormat(request.content.core_security_permissions_grid_header_name)#',width:30,sortable:true,dataIndex:'name'},{header:'#jsStringFormat(request.content.core_security_permissions_grid_header_module)#',width:30,sortable:true,dataIndex:'module'},
	        	action
	        ]),
	        sm: sm,
	        ...
	        // inline toolbars
	        tbar:[{
	            text:'#jsStringFormat(request.content.core_security_permissions_grid_global_add)#',
	            tooltip:'#jsStringFormat(request.content.core_security_permissions_grid_global_add)#',
	            iconCls:'add',
	            handler:function(){window.location.href='#myself##xfa.add#';}
	        },'-',{
	            text:'#jsStringFormat(request.content.core_security_permissions_grid_global_delete)#',
	            tooltip:'#jsStringFormat(request.content.core_security_permissions_grid_global_delete)#',
	            iconCls:'remove',
	            handler: doDel
	        }],
	        ...
	        height:533,
	        frame:false,
	        renderTo: Ext.get('grid_core_security_permissions'),
	    ...
	    function doDel(){
	        ...
	        if(m.length > 0) Ext.MessageBox.confirm('Message','#jsStringFormat(request.content.core_security_permissions_grid_global_delete_warning_confirm)#',doDel2);
	        else Ext.MessageBox.alert('Message','#jsStringFormat(request.content.core_security_permissions_grid_global_delete_warning_select)#');
	    }     
...
<div id="grid_core_security_permissions"></div>
</cfoutput>
