...
<cfparam name="variables.fieldlist" default="user_id,role_id">
</cfsilent>
<cfoutput>
...
<script type="text/javascript">
	Ext.onReady(function(){
		...
		action.on({
			action:function(grid, record, action, row, col) {
				if(action == 'icon-edit-record'){
					window.location.href='#application.lanshock.oHelper.buildUrl('#xfa.update#')#&role_id=' + grid.getSelectionModel().getSelected().id;
				}
	    ...
	    var ds = new Ext.data.GroupingStore({
			...
			reader: new Ext.data.JsonReader({totalProperty: 'totalRecords', root: 'data', id: 'role_id'},[
		...
		var grid = new xg.GridPanel({
	        ...
	        viewConfig: {forceFit: true}, renderTo: Ext.get('grid_core_security_users_roles_rel'),
	        bbar: new Ext.PagingToolbar({pageSize: 20, store: ds, displayInfo: true}),
			cm: new xg.ColumnModel([
	        	sm,
	        	{id:'role_id',header:'#jsStringFormat(request.content.core_security_users_roles_rel_grid_header_user_id)#',sortable:true,dataIndex:'user_id',width:30},{header:'#jsStringFormat(request.content.core_security_users_roles_rel_grid_header_role_id)#',sortable:true,dataIndex:'role_id',width:30},
	        	action
	        ]),
	    ...
	    function doDel2(btn){
	       if(btn == 'yes') {	
		        ...
		        for(var i = 0, len = m.length; i < len; i++){        		
					var ss = "{\"role_id\":\"" + m[i].get("role_id") + "\"}";
