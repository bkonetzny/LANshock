...
<script type="text/javascript">
	Ext.onReady(function(){
        ...
        var action = new Ext.ux.grid.RowActions({
			header:'#jsStringFormat(request.content.news_category_grid_header__rowactions)#',
			actions:[{
				iconCls:'icon-edit-record',
				tooltip:'#jsStringFormat(request.content.news_category_grid_row_edit)#'
			},{
				iconCls:'icon-delete-record',
				tooltip:'#jsStringFormat(request.content.news_category_grid_row_delete)#'
			}]
		});
		...
		var grid = new xg.GridPanel({
	        ...
	        viewConfig: {forceFit: true}, renderTo: Ext.get('grid_news_category'),
	        bbar: new Ext.PagingToolbar({pageSize: 20, store: ds, displayInfo: true}),
			cm: new xg.ColumnModel([
	        	sm,
	        	{id:'id',header:'#jsStringFormat(request.content.news_category_grid_header_id)#',sortable:true,dataIndex:'id',width:30},{header:'#jsStringFormat(request.content.news_category_grid_header_name)#',sortable:true,dataIndex:'name',width:30},
	        	action
	        ]),
