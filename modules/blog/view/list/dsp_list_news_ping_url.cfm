<cfsilent>
<!--- -->
<fusedoc fuse="$RCSfile: dsp_list_news_ping_url.cfm,v $" language="ColdFusion 7.01" version="2.0"  >
	<responsibilities>
		This page displays a listing of the news_ping_url records from a recordset. 
		Each record has a link for viewing, deleting, and editing the selected news_ping_url record.
	</responsibilities>
	<properties>
	...
	<io>
		<in>
			...
			<number name="_totalRowCount" precision="integer" scope="variables" comments="Count of rows in the news_ping_url table." />
			<string name="_listSortByFieldList" default="">
			
			<recordset name="qnews_ping_url" primaryKeys="id" scope="variables" comments="Recordset containing news_ping_url records " >
	...
	Ext.onReady(function(){
	    ...
	    var grid = new xg.GridPanel({
	        ...
	        cm: new xg.ColumnModel([
	        	sm,
	        	{id:'id',header:'#jsStringFormat(request.content.news_ping_url_grid_header_id)#',width:30,sortable:true,dataIndex:'id'},{header:'#jsStringFormat(request.content.news_ping_url_grid_header_name)#',width:30,sortable:true,dataIndex:'name'},{header:'#jsStringFormat(request.content.news_ping_url_grid_header_url)#',width:30,sortable:true,dataIndex:'url'},
	        	action
	        ]),
	        sm: sm,
	        ...
	        // inline toolbars
	        tbar:[{
	            text:'#jsStringFormat(request.content.news_ping_url_grid_global_add)#',
	            tooltip:'#jsStringFormat(request.content.news_ping_url_grid_global_add)#',
	            iconCls:'add',
	            handler:function(){window.location.href='#myself##xfa.add#';}
	        },'-',{
	            text:'#jsStringFormat(request.content.news_ping_url_grid_global_delete)#',
	            tooltip:'#jsStringFormat(request.content.news_ping_url_grid_global_delete)#',
	            iconCls:'remove',
	            handler: doDel
	        }],
	        ...
	        height:533,
	        frame:false,
	        renderTo: Ext.get('grid_news_ping_url'),
	    ...
	    function doDel(){
	        ...
	        if(m.length > 0) Ext.MessageBox.confirm('Message','#jsStringFormat(request.content.news_ping_url_grid_global_delete_warning_confirm)#',doDel2);
	        else Ext.MessageBox.alert('Message','#jsStringFormat(request.content.news_ping_url_grid_global_delete_warning_select)#');
	    }     
