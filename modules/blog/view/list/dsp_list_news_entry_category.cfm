...
<script type="text/javascript">
	Ext.onReady(function(){
        ...
        var action = new Ext.ux.grid.RowActions({
			header:'#jsStringFormat(request.content.news_entry_category_grid_header__rowactions)#',
			actions:[{
				iconCls:'icon-edit-record',
				tooltip:'#jsStringFormat(request.content.news_entry_category_grid_row_edit)#'
			},{
				iconCls:'icon-delete-record',
				tooltip:'#jsStringFormat(request.content.news_entry_category_grid_row_delete)#'
			}]
		});
		...
		var grid = new xg.GridPanel({
	        ...
	        viewConfig: {forceFit: true}, renderTo: Ext.get('grid_news_entry_category'),
	        bbar: new Ext.PagingToolbar({pageSize: 20, store: ds, displayInfo: true}),
			cm: new xg.ColumnModel([
	        	sm,
	        	{id:'id',header:'#jsStringFormat(request.content.news_entry_category_grid_header_id)#',sortable:true,dataIndex:'id',width:30},{header:'#jsStringFormat(request.content.news_entry_category_grid_header_entry_id)#',sortable:true,dataIndex:'entry_id',width:30},{header:'#jsStringFormat(request.content.news_entry_category_grid_header_category_id)#',sortable:true,dataIndex:'category_id',width:30},
	        	action
	        ]),
