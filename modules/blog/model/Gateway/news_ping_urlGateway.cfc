<cfcomponent displayname="news_ping_urlGateway.cfc" extends="reactor.project.lanshock.Gateway.news_ping_urlGateway">
...
<cffunction name="getRecords" access="public" hint="I return all matching rows from the table." output="false" returntype="query">
		...
		<cfif NOT StructIsEmpty(arguments.stFilter)>
			...
			<cfif StructKeyExists(arguments.stFilter,'stJoins')>
				<cfset Query.returnObjectFields("news_ping_url","id,name,url")>
				
				<cfloop collection="#arguments.stFilter.stJoins#" item="idx">
					<cfset idx = lCase(idx)>
					<cfset Query.leftJoin("news_ping_url",idx,idx)>
			...
			<cfif StructKeyExists(arguments.stFilter,'stFields')>
				<cfloop collection="#arguments.stFilter.stFields#" item="idx">
					...
					<cfelse>
						<cfset sCurrentTable = "news_ping_url">
			...
			<cfif StructKeyExists(arguments.stFilter,'lSortFields')>
				<cfloop list="#arguments.stFilter.lSortFields#" index="idx">
					...
					<cfif ListLen(idx,'|') EQ 2>
						<cfset idx = ListPrepend(idx,"news_ping_url",'|')>
					</cfif>
	...
	<cffunction name="getNWithJoin" output="false" returntype="query" hint="I get the selected N records, with an outer join to any parent tables.">
		...
		<cfif arguments.startrow GT 1>
			...
			<cfset QuerySkip.returnObjectFields("news_ping_url","id") />
			<cfset QuerySkip.setMaxrows(arguments.startrow) />
			<cfset qSkip = getByQuery(QuerySkip) />
			<cfset where.isNotIn("news_ping_url","id",valuelist(qSkip.id)) />
		</cfif>
		...
		<cfif isStruct(arguments.filter)>
			<cfset QueryRecordset.getWhere().setMode("OR")>
				...
				<cfif StructKeyExists(arguments.filter,'id')>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("news_ping_url",'id',arguments.filter['id'])>
					</cfif>
				</cfif>
				...
				<cfif StructKeyExists(arguments.filter,'name')>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("news_ping_url",'name',arguments.filter['name'])>
					</cfif>
				</cfif>
				...
				<cfif StructKeyExists(arguments.filter,'url')>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("news_ping_url",'url',arguments.filter['url'])>
					</cfif>
				</cfif>
		...
		<cfset QueryRecordset.returnObjectFields("news_ping_url","id,name,url")>
	...
	<cffunction name="getOptions" access="remote" output="false" returntype="query" hint="I get the option values for this table.">
		...
		<cfif StructKeyExists(variables,'sOptionNameCode')>
			<cfset QueryRecordset.setFieldExpression("news_ping_url",variables.sOptionNameField,variables.sOptionNameCode)>
		</cfif>
		<cfset QueryRecordset.setFieldAlias("news_ping_url","id","optionvalue")>
		<cfset QueryRecordset.setFieldAlias("news_ping_url",variables.sOptionNameField,"optionname")>
		<cfset QueryRecordset.returnObjectFields("news_ping_url","optionvalue,optionname")>
	...
	<cffunction name="getRecordCount" output="false" returntype="numeric" hint="I get the number of records in the table.">
		...
		<cfset QueryCount.returnObjectFields("news_ping_url","id") />
		<cfset QueryCount.setFieldExpression("news_ping_url","id","COUNT(*)","CF_SQL_INTEGER") />
