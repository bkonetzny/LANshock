<cfcomponent displayname="content_contentGateway.cfc" extends="reactor.project.lanshock.Gateway.content_contentGateway">
<!--- Start of gatewayCustom code generated by fusebox scaffolder, it will be replaced if fusebox scaffolder is rerun. --->
<cfset variables.sOptionNameField = "codename">
	...
	<cffunction name="getRecords" access="public" hint="I return all matching rows from the table." output="false" returntype="query">
		...
		<cfif NOT StructIsEmpty(arguments.stFilter)>
			...
			<cfif StructKeyExists(arguments.stFilter,'stJoins')>
				<cfset Query.returnObjectFields("content_content","id,codename,title,content,user_id,dtcreated,dtchanged,bactive")>
				
				<cfloop collection="#arguments.stFilter.stJoins#" item="idx">
					<cfset idx = lCase(idx)>
					<cfset Query.leftJoin("content_content",idx,idx)>
			...
			<cfif StructKeyExists(arguments.stFilter,'stFields')>
				<cfloop collection="#arguments.stFilter.stFields#" item="idx">
					...
					<cfelse>
						<cfset sCurrentTable = "content_content">
			...
			<cfif StructKeyExists(arguments.stFilter,'lSortFields')>
				<cfloop list="#arguments.stFilter.lSortFields#" index="idx">
					...
					<cfif ListLen(idx,'|') EQ 2>
						<cfset idx = ListPrepend(idx,"content_content",'|')>
					</cfif>
	...
	<cffunction name="getAllWithJoin" output="false" returntype="query" hint="I get ALL the records, with an outer join to any parent tables.">
		...
		<cfset Query.leftJoin("content_content", "user", "user")/>
	...
	<cffunction name="getNWithJoin" output="false" returntype="query" hint="I get the selected N records, with an outer join to any parent tables.">
		...
		<cfif arguments.startrow GT 1>
			...
			<cfset QuerySkip.returnObjectFields("content_content","id") />
			<cfset QuerySkip.setMaxrows(arguments.startrow) />
			<cfset qSkip = getByQuery(QuerySkip) />
			<cfset where.isNotIn("content_content","id",valuelist(qSkip.id)) />
		</cfif>
	  
		<!--- Left join all the associated tables (uses a left join for safety). --->
		<!--- Syntax is: Query.leftJoin(joinFromObjectAlias,joinToObjectAlias,relationshipAlias) --->
	  
		<cfset QueryRecordset.leftJoin("content_content", "user", "user")/>
		...
		<cfif isStruct(arguments.filter)>
			<cfset QueryRecordset.getWhere().setMode("OR")>
				...
				<cfif StructKeyExists(arguments.filter,'id')>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("content_content",'id',arguments.filter['id'])>
					</cfif>
				</cfif>
			
				<cfif StructKeyExists(arguments.filter,'codename')>
					<cfset bFilterColumn = false>
					<cfswitch expression="varchar">
						<cfcase value="numeric">
							<cfif isNumeric(arguments.filter['codename'])>
								<cfset bFilterColumn = true>
							</cfif>
						</cfcase>
						<cfdefaultcase>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("content_content",'codename',arguments.filter['codename'])>
					</cfif>
				</cfif>
				...
				<cfif StructKeyExists(arguments.filter,'title')>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("content_content",'title',arguments.filter['title'])>
					</cfif>
				</cfif>
			
				<cfif StructKeyExists(arguments.filter,'content')>
					<cfset bFilterColumn = false>
					<cfswitch expression="text">
						<cfcase value="numeric">
							<cfif isNumeric(arguments.filter['content'])>
								<cfset bFilterColumn = true>
							</cfif>
						</cfcase>
						<cfdefaultcase>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("content_content",'content',arguments.filter['content'])>
					</cfif>
				</cfif>
			
				<cfif StructKeyExists(arguments.filter,'user_id')>
					<cfset bFilterColumn = false>
					<cfswitch expression="integer">
						<cfcase value="numeric">
							<cfif isNumeric(arguments.filter['user_id'])>
								<cfset bFilterColumn = true>
							</cfif>
						</cfcase>
						<cfdefaultcase>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("content_content",'user_id',arguments.filter['user_id'])>
					</cfif>
				</cfif>
			
				<cfif StructKeyExists(arguments.filter,'dtcreated')>
					<cfset bFilterColumn = false>
					<cfswitch expression="datetime">
						<cfcase value="numeric">
							<cfif isNumeric(arguments.filter['dtcreated'])>
								<cfset bFilterColumn = true>
							</cfif>
						</cfcase>
						<cfdefaultcase>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("content_content",'dtcreated',arguments.filter['dtcreated'])>
					</cfif>
				</cfif>
			
				<cfif StructKeyExists(arguments.filter,'dtchanged')>
					<cfset bFilterColumn = false>
					<cfswitch expression="datetime">
						<cfcase value="numeric">
							<cfif isNumeric(arguments.filter['dtchanged'])>
								<cfset bFilterColumn = true>
							</cfif>
						</cfcase>
						<cfdefaultcase>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("content_content",'dtchanged',arguments.filter['dtchanged'])>
					</cfif>
				</cfif>
			
				<cfif StructKeyExists(arguments.filter,'bactive')>
					<cfset bFilterColumn = false>
					<cfswitch expression="boolean">
						<cfcase value="numeric">
							<cfif isNumeric(arguments.filter['bactive'])>
								<cfset bFilterColumn = true>
							</cfif>
						</cfcase>
						<cfdefaultcase>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("content_content",'bactive',arguments.filter['bactive'])>
					</cfif>
				</cfif>
		...
		<cfset QueryRecordset.returnObjectFields("content_content","id,codename,title,user_id,dtchanged,bactive")>
	...
	<cffunction name="getOptions" access="remote" output="false" returntype="query" hint="I get the option values for this table.">
		...
		<cfif len(variables.sOptionNameCode)>
			<cfset QueryRecordset.setFieldExpression("content_content",variables.sOptionNameField,variables.sOptionNameCode)>
		</cfif>
		<cfset QueryRecordset.setFieldAlias("content_content","id","optionvalue")>
		<cfset QueryRecordset.setFieldAlias("content_content",variables.sOptionNameField,"optionname")>
		<cfset QueryRecordset.returnObjectFields("content_content","optionvalue,optionname")>
	...
	<cffunction name="getRecordCount" output="false" returntype="numeric" hint="I get the number of records in the table.">
		...
		<cfset QueryCount.returnObjectFields("content_content","id") />
		<cfset QueryCount.setFieldExpression("content_content","id","COUNT(*)","CF_SQL_INTEGER") />
