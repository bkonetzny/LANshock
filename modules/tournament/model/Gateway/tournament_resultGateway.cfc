<cfcomponent displayname="tournament_resultGateway.cfc" extends="reactor.project.lanshock.Gateway.tournament_resultGateway">
<!--- Start of gatewayCustom code generated by fusebox scaffolder, it will be replaced if fusebox scaffolder is rerun. --->
<cfset variables.sOptionNameField = "matchid">
	...
	<cffunction name="getRecords" access="public" hint="I return all matching rows from the table." output="false" returntype="query">
		...
		<cfif NOT StructIsEmpty(arguments.stFilter)>
			...
			<cfif StructKeyExists(arguments.stFilter,'stJoins')>
				<cfset Query.returnObjectFields("tournament_result","id,matchid,team1_result,team2_result")>
				
				<cfloop collection="#arguments.stFilter.stJoins#" item="idx">
					<cfset idx = lCase(idx)>
					<cfset Query.leftJoin("tournament_result",idx,idx)>
			...
			<cfif StructKeyExists(arguments.stFilter,'stFields')>
				<cfloop collection="#arguments.stFilter.stFields#" item="idx">
					...
					<cfelse>
						<cfset sCurrentTable = "tournament_result">
			...
			<cfif StructKeyExists(arguments.stFilter,'lSortFields')>
				<cfloop list="#arguments.stFilter.lSortFields#" index="idx">
					...
					<cfif ListLen(idx,'|') EQ 2>
						<cfset idx = ListPrepend(idx,"tournament_result",'|')>
					</cfif>
	...
	<cffunction name="getAllWithJoin" output="false" returntype="query" hint="I get ALL the records, with an outer join to any parent tables.">
		...
		<cfset Query.leftJoin("tournament_result", "tournament_type_se_match", "tournament_type_se_match")/>
		<cfset Query.setFieldPrefix("tournament_type_se_match","tournament_type_se_match_")/>
	...
	<cffunction name="getNWithJoin" output="false" returntype="query" hint="I get the selected N records, with an outer join to any parent tables.">
		...
		<cfif arguments.startrow GT 1>
			...
			<cfset QuerySkip.returnObjectFields("tournament_result","id") />
			<cfset QuerySkip.setMaxrows(arguments.startrow) />
			<cfset qSkip = getByQuery(QuerySkip) />
			<cfset where.isNotIn("tournament_result","id",valuelist(qSkip.id)) />
		</cfif>
	  
		<!--- Left join all the associated tables (uses a left join for safety). --->
		<!--- Syntax is: Query.leftJoin(joinFromObjectAlias,joinToObjectAlias,relationshipAlias) --->
	  
		<cfset QueryRecordset.leftJoin("tournament_result", "tournament_type_se_match", "tournament_type_se_match")/>
		<cfset QueryRecordset.setFieldPrefix("tournament_type_se_match","tournament_type_se_match_")/>
		...
		<cfif isStruct(arguments.filter)>
			<cfset QueryRecordset.getWhere().setMode("OR")>
				...
				<cfif StructKeyExists(arguments.filter,'id')>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("tournament_result",'id',arguments.filter['id'])>
					</cfif>
				</cfif>
			
				<cfif StructKeyExists(arguments.filter,'matchid')>
					
							<cfset bFilterColumn = false>
							<cfif isNumeric(arguments.filter['matchid'])>
								<cfset bFilterColumn = true>
							</cfif>
						
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("tournament_result",'matchid',arguments.filter['matchid'])>
					</cfif>
				</cfif>
			
				<cfif StructKeyExists(arguments.filter,'team1_result')>
					
							<cfset bFilterColumn = false>
							<cfif isNumeric(arguments.filter['team1_result'])>
								<cfset bFilterColumn = true>
							</cfif>
						
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("tournament_result",'team1_result',arguments.filter['team1_result'])>
					</cfif>
				</cfif>
			
				<cfif StructKeyExists(arguments.filter,'team2_result')>
					
							<cfset bFilterColumn = false>
							<cfif isNumeric(arguments.filter['team2_result'])>
								<cfset bFilterColumn = true>
							</cfif>
						
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("tournament_result",'team2_result',arguments.filter['team2_result'])>
					</cfif>
				</cfif>
		...
		<cfset QueryRecordset.returnObjectFields("tournament_result","id,matchid,team1_result,team2_result")>
	...
	<cffunction name="getOptions" access="remote" output="false" returntype="query" hint="I get the option values for this table.">
		...
		<cfif len(variables.sOptionNameCode)>
			<cfset QueryRecordset.setFieldExpression("tournament_result",variables.sOptionNameField,variables.sOptionNameCode)>
		</cfif>
		<cfset QueryRecordset.setFieldAlias("tournament_result","id","optionvalue")>
		<cfset QueryRecordset.setFieldAlias("tournament_result",variables.sOptionNameField,"optionname")>
		<cfset QueryRecordset.returnObjectFields("tournament_result","optionvalue,optionname")>
	...
	<cffunction name="getRecordCount" output="false" returntype="numeric" hint="I get the number of records in the table.">
		...
		<cfset QueryCount.returnObjectFields("tournament_result","id") />
		<cfset QueryCount.setFieldExpression("tournament_result","id","COUNT(*)","CF_SQL_INTEGER") />
