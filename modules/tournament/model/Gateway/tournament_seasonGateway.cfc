<cfcomponent displayname="tournament_seasonGateway.cfc" extends="reactor.project.lanshock.Gateway.tournament_seasonGateway">
...
<cfset variables.sOptionNameField = "name">
	...
	<cffunction name="getRecords" access="public" hint="I return all matching rows from the table." output="false" returntype="query">
		...
		<cfif NOT StructIsEmpty(arguments.stFilter)>
			...
			<cfif StructKeyExists(arguments.stFilter,'stJoins')>
				<cfset Query.returnObjectFields("tournament_season","id,name,description,event_id,player_coins,dt_start,dt_end")>
				
				<cfloop collection="#arguments.stFilter.stJoins#" item="idx">
					<cfset idx = lCase(idx)>
					<cfset Query.leftJoin("tournament_season",idx,idx)>
			...
			<cfif StructKeyExists(arguments.stFilter,'stFields')>
				<cfloop collection="#arguments.stFilter.stFields#" item="idx">
					...
					<cfelse>
						<cfset sCurrentTable = "tournament_season">
			...
			<cfif StructKeyExists(arguments.stFilter,'lSortFields')>
				<cfloop list="#arguments.stFilter.lSortFields#" index="idx">
					...
					<cfif ListLen(idx,'|') EQ 2>
						<cfset idx = ListPrepend(idx,"tournament_season",'|')>
					</cfif>
	...
	<cffunction name="getAllWithJoin" output="false" returntype="query" hint="I get ALL the records, with an outer join to any parent tables.">
		...
		<cfset Query.leftJoin("tournament_season", "event_events", "event_events")/>
		<cfset Query.setFieldPrefix("event_events","event_events_")/>
	...
	<cffunction name="getNWithJoin" output="false" returntype="query" hint="I get the selected N records, with an outer join to any parent tables.">
		...
		<cfif arguments.startrow GT 1>
			...
			<cfset QuerySkip.returnObjectFields("tournament_season","id") />
			<cfset QuerySkip.setMaxrows(arguments.startrow) />
			<cfset qSkip = getByQuery(QuerySkip) />
			<cfset where.isNotIn("tournament_season","id",valuelist(qSkip.id)) />
		</cfif>
	  
		<!--- Left join all the associated tables (uses a left join for safety). --->
		<!--- Syntax is: Query.leftJoin(joinFromObjectAlias,joinToObjectAlias,relationshipAlias) --->
	  
		<cfset QueryRecordset.leftJoin("tournament_season", "event_events", "event_events")/>
		<cfset QueryRecordset.setFieldPrefix("event_events","event_events_")/>
		...
		<cfif isStruct(arguments.filter)>
			<cfset QueryRecordset.getWhere().setMode("OR")>
				...
				<cfif StructKeyExists(arguments.filter,'id')>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("tournament_season",'id',arguments.filter['id'])>
					</cfif>
				</cfif>
				...
				<cfif StructKeyExists(arguments.filter,'name')>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("tournament_season",'name',arguments.filter['name'])>
					</cfif>
				</cfif>
				...
				<cfif StructKeyExists(arguments.filter,'description')>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("tournament_season",'description',arguments.filter['description'])>
					</cfif>
				</cfif>
			
				<cfif StructKeyExists(arguments.filter,'event_id')>
					
							<cfset bFilterColumn = false>
							<cfif isNumeric(arguments.filter['event_id'])>
								<cfset bFilterColumn = true>
							</cfif>
						
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("tournament_season",'event_id',arguments.filter['event_id'])>
					</cfif>
				</cfif>
			
				<cfif StructKeyExists(arguments.filter,'player_coins')>
					
							<cfset bFilterColumn = false>
							<cfif isNumeric(arguments.filter['player_coins'])>
								<cfset bFilterColumn = true>
							</cfif>
						
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("tournament_season",'player_coins',arguments.filter['player_coins'])>
					</cfif>
				</cfif>
			
				<cfif StructKeyExists(arguments.filter,'dt_start')>
					
							<cfset bFilterColumn = false>
							<cfif LsIsDate(arguments.filter['dt_start'])>
								<cfset bFilterColumn = true>
							</cfif>
						
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("tournament_season",'dt_start',arguments.filter['dt_start'])>
					</cfif>
				</cfif>
			
				<cfif StructKeyExists(arguments.filter,'dt_end')>
					
							<cfset bFilterColumn = false>
							<cfif LsIsDate(arguments.filter['dt_end'])>
								<cfset bFilterColumn = true>
							</cfif>
						
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("tournament_season",'dt_end',arguments.filter['dt_end'])>
					</cfif>
				</cfif>
		...
		<cfset QueryRecordset.returnObjectFields("tournament_season","id,name,event_id,dt_start,dt_end")>
	...
	<cffunction name="getOptions" access="remote" output="false" returntype="query" hint="I get the option values for this table.">
		...
		<cfif len(variables.sOptionNameCode)>
			<cfset QueryRecordset.setFieldExpression("tournament_season",variables.sOptionNameField,variables.sOptionNameCode)>
		</cfif>
		<cfset QueryRecordset.setFieldAlias("tournament_season","id","optionvalue")>
		<cfset QueryRecordset.setFieldAlias("tournament_season",variables.sOptionNameField,"optionname")>
		<cfset QueryRecordset.returnObjectFields("tournament_season","optionvalue,optionname")>
	...
	<cffunction name="getRecordCount" output="false" returntype="numeric" hint="I get the number of records in the table.">
		...
		<cfset QueryCount.returnObjectFields("tournament_season","id") />
		<cfset QueryCount.setFieldExpression("tournament_season","id","COUNT(*)","CF_SQL_INTEGER") />
