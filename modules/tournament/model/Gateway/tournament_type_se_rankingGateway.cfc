<cfcomponent displayname="tournament_type_se_rankingGateway.cfc" extends="reactor.project.lanshock.Gateway.tournament_type_se_rankingGateway">
...
<cfset variables.sOptionNameField = "tournamentid">
	...
	<cffunction name="getRecords" access="public" hint="I return all matching rows from the table." output="false" returntype="query">
		...
		<cfif NOT StructIsEmpty(arguments.stFilter)>
			...
			<cfif StructKeyExists(arguments.stFilter,'stJoins')>
				<cfset Query.returnObjectFields("tournament_type_se_ranking","id,tournamentid,teamid,pos")>
				
				<cfloop collection="#arguments.stFilter.stJoins#" item="idx">
					<cfset idx = lCase(idx)>
					<cfset Query.leftJoin("tournament_type_se_ranking",idx,idx)>
			...
			<cfif StructKeyExists(arguments.stFilter,'stFields')>
				<cfloop collection="#arguments.stFilter.stFields#" item="idx">
					...
					<cfelse>
						<cfset sCurrentTable = "tournament_type_se_ranking">
			...
			<cfif StructKeyExists(arguments.stFilter,'lSortFields')>
				<cfloop list="#arguments.stFilter.lSortFields#" index="idx">
					...
					<cfif ListLen(idx,'|') EQ 2>
						<cfset idx = ListPrepend(idx,"tournament_type_se_ranking",'|')>
					</cfif>
	...
	<cffunction name="getNWithJoin" output="false" returntype="query" hint="I get the selected N records, with an outer join to any parent tables.">
		...
		<cfif arguments.startrow GT 1>
			...
			<cfset QuerySkip.returnObjectFields("tournament_type_se_ranking","id") />
			<cfset QuerySkip.setMaxrows(arguments.startrow) />
			<cfset qSkip = getByQuery(QuerySkip) />
			<cfset where.isNotIn("tournament_type_se_ranking","id",valuelist(qSkip.id)) />
		</cfif>
		...
		<cfif isStruct(arguments.filter)>
			<cfset QueryRecordset.getWhere().setMode("OR")>
				...
				<cfif StructKeyExists(arguments.filter,'id')>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("tournament_type_se_ranking",'id',arguments.filter['id'])>
					</cfif>
				</cfif>
				...
				<cfif StructKeyExists(arguments.filter,'tournamentid')>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("tournament_type_se_ranking",'tournamentid',arguments.filter['tournamentid'])>
					</cfif>
				</cfif>
				...
				<cfif StructKeyExists(arguments.filter,'teamid')>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("tournament_type_se_ranking",'teamid',arguments.filter['teamid'])>
					</cfif>
				</cfif>
				...
				<cfif StructKeyExists(arguments.filter,'pos')>
					...
					<cfif bFilterColumn>
						<cfset QueryRecordset.getWhere().isLike("tournament_type_se_ranking",'pos',arguments.filter['pos'])>
					</cfif>
				</cfif>
		...
		<cfset QueryRecordset.returnObjectFields("tournament_type_se_ranking","id,tournamentid,teamid,pos")>
	...
	<cffunction name="getOptions" access="remote" output="false" returntype="query" hint="I get the option values for this table.">
		...
		<cfif len(variables.sOptionNameCode)>
			<cfset QueryRecordset.setFieldExpression("tournament_type_se_ranking",variables.sOptionNameField,variables.sOptionNameCode)>
		</cfif>
		<cfset QueryRecordset.setFieldAlias("tournament_type_se_ranking","id","optionvalue")>
		<cfset QueryRecordset.setFieldAlias("tournament_type_se_ranking",variables.sOptionNameField,"optionname")>
		<cfset QueryRecordset.returnObjectFields("tournament_type_se_ranking","optionvalue,optionname")>
	...
	<cffunction name="getRecordCount" output="false" returntype="numeric" hint="I get the number of records in the table.">
		...
		<cfset QueryCount.returnObjectFields("tournament_type_se_ranking","id") />
		<cfset QueryCount.setFieldExpression("tournament_type_se_ranking","id","COUNT(*)","CF_SQL_INTEGER") />
