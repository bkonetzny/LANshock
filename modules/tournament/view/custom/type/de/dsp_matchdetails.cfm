...
<cfoutput>
	<cfif session.oUser.checkPermissions('manage')>
		<h4>#request.content.type_de_matchdetails_statuschange#</h4>
		...
		<ul class="options">
			<cfif stMatch.status EQ 'empty'>
				<li><strong>#request.content.type_de_matchdetails_status_save_teams#</strong></li>
			<cfelse>
				<li><a href="#urlstring#empty">#request.content.type_de_matchdetails_status_save_teams#</a></li>
			</cfif>
			<cfif stMatch.status EQ 'play'>
				<li><strong>#request.content.type_de_matchdetails_status_playing#</strong></li>
			<cfelse>
				<li><a href="#urlstring#play">#request.content.type_de_matchdetails_status_playing#</a></li>
			</cfif>
			<cfif stMatch.status EQ 'admincheck'>
				<li><strong>#request.content.type_de_matchdetails_status_admincheck#</strong></li>
			<cfelse>
				<li><a href="#urlstring#admincheck">#request.content.type_de_matchdetails_status_admincheck#</a></li>
			</cfif>
			<cfif stMatch.status EQ 'done'>
				<li><strong>#request.content.type_de_matchdetails_status_done#</strong></li>
			<cfelse>
				<li><a href="#urlstring#done">#request.content.type_de_matchdetails_status_done#</a></li>
			</cfif>
			<li><a href="#urlstring#reset">#request.content.type_de_matchdetails_status_reset#</a></li>
		</ul>
	</cfif>
	...
	<cfif stMatch.status EQ 'empty' AND session.oUser.checkPermissions('manage')>
		<form action="#application.lanshock.oHelper.buildUrl('#myfusebox.thiscircuit#.#myfusebox.thisfuseaction#')#" method="post" class="uniForm">
			...
			<div class="buttonHolder">
				<button type="submit" class="submitButton">#request.content.type_de_matchdetails_saveassign#</button>
			</div>
		</form>
	<cfelse>
		<div style="float: left; width: 50%; text-align: center;">
			...
			<cfif stMatch.winner EQ keyTeam>
				...
				<cfset teamstatus = request.content.type_de_status_team_won>
			...
			<cfelse>
				...
				<cfif len(stMatch.winner)>
					...
					<cfset teamstatus = request.content.type_de_status_team_lost>
				<cfelse>
	...
	<cfif stMatch.qResults.recordcount>
		<h4>#request.content.type_de_matchdetails_results#</h4>
		...
		<cfelse>
			<p><img src="#application.lanshock.oRuntime.getEnvironment().sWebPath#templates/_shared/images/famfamfam/icons/error.png" alt="" /> #request.content.type_de_results_to_be_checked#</p>
		</cfif>
		...
		<table class="vlist">
			...
			<cfloop query="stMatch.qResults">
				<tr>
					...
					<td align="center">#team1_result# #request.content.type_de_resultdelimiter# #team2_result#</td>
				</tr>
			</cfloop>
		</table>
		...
		<cfif ListFind('#stMatch.team1.id#,#stMatch.team2.id#',qTeamCurrentUser.id) AND qTeamCurrentUser.id NEQ stMatch.submittedby_teamid>
			<form action="#application.lanshock.oHelper.buildUrl('#myfusebox.thiscircuit#.#myfusebox.thisfuseaction#')#" method="post" class="uniForm">
				...
				<div class="buttonHolder">
					<button name="check1" type="submit" class="submitButton">#request.content.type_de_submit_results_check1#</button>
					<button name="check0" type="submit" class="cancelButton">#request.content.type_de_submit_results_check0#</button>
				</div>
			</form>
		</cfif>
	</cfif>
	...
	<cfif (qTournament.status EQ 'playing' AND stMatch.status EQ 'play' AND ListFind('#stMatch.team1.id#,#stMatch.team2.id#',qTeamCurrentUser.id)) OR (session.oUser.checkPermissions('manage') AND stMatch.status EQ 'admincheck')>
		<form id="form_results" style="display: none;" action="#application.lanshock.oHelper.buildUrl('#myfusebox.thiscircuit#.#myfusebox.thisfuseaction#')#" method="post" class="uniForm" onsubmit="return LANshock.Modules.oTournament.oTypeSE.checkResultsForm();">
			...
			<fieldset class="inlineLabels">
				<legend>#request.content.type_de_matchdetails_results#</legend>
				...
				<cfloop query="stMatch.qResults">
					<div class="ctrlHolder">
						...
						<span style="float: left;">#request.content.type_de_resultdelimiter#</span>
		...
		<form id="form_results_pre" action="##" method="post" class="uniForm">
			<div class="buttonHolder">
				<button type="submit" class="submitButton" onclick="$('##form_results').toggle();$('##form_results_pre').toggle();return false;">#request.content.type_de_submit_results#</button>
			</div>
		</form>
	</cfif>
	...
	<cfif qTournament.status EQ 'playing' AND session.oUser.checkPermissions('manage') AND stMatch.status EQ 'admincheck'>
		<form action="#application.lanshock.oHelper.buildUrl('#myfusebox.thiscircuit#.#myfusebox.thisfuseaction#')#" method="post" class="uniForm">
			...
			<fieldset class="inlineLabels">
				<legend>#request.content.type_de_matchdetails_set_winner#</legend>
			</fieldset>
			
			<div class="buttonHolder">
				<button type="submit" class="submitButton" onclick="$('##winner').val('team1');"<!--- <cfif stMatch.winner EQ 'team1'> disabled="disabled"</cfif> --->>#request.content.type_de_matchdetails_team1_winner#</button>
				<button type="submit" class="resetButton" onclick="$('##winner').val('NULL');"<!--- <cfif NOT len(stMatch.winner)> disabled="disabled"</cfif> --->>#request.content.type_de_matchdetails_no_winner#</button>
				<button type="submit" class="submitButton" onclick="$('##winner').val('team2');"<!--- <cfif stMatch.winner EQ 'team2'> disabled="disabled"</cfif> --->>#request.content.type_de_matchdetails_team2_winner#</button>
			</div>
		</form>
	</cfif>

	<h4>#request.content.type_de_matchdetails_info#</h4>
	<table class="vlist">
		<tr>
			<th>#request.content.type_de_matchdetails_info_status#</th>
		...
		<tr>
			<th>#request.content.type_de_matchdetails_info_matchstatus#</th>
			<td>
		...
		<cfif qTournament.teamsize EQ 1>
			<tr>
				<th>#request.content.type_de_matchdetails_info_result1#</th>
				<td>
			...
			<tr>
				<th>#request.content.type_de_matchdetails_info_result2#</th>
				<td>
					<cfif len(stMatch.checkedby_userid)>
		...
		<tr>
			<th>#request.content.type_de_matchdetails_info_result3#</th>
			<td>
