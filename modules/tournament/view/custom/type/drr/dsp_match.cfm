	...
	<cfif session.oUser.checkPermissions('manage')
		...
		<ul class="options">
			<cfif session.oUser.checkPermissions('manage') AND NOT ListFindNoCase('signup,warmup',qTournament.status)>
				<li><a href="##" onclick="LANshock.Modules.oTournament.oTypeDRR.showMatchStatus();return false;">#request.content.type_se_match_markstatus#</a></li>
			</cfif>
			<cfif qTeamCurrentUser.recordcount AND NOT ListFindNoCase('signup,warmup',qTournament.status)>
				<li><a href="##" onclick="LANshock.Modules.oTournament.oTypeDRR.markTeam('#qTeamCurrentUser.id#');return false;">#request.content.type_se_match_markteam#</a></li>
			</cfif>
		</ul>
	</cfif>
</cfoutput>
