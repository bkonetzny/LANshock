...
date:		1-April-2004
revisions:	2005-06-20 bkonetzny (added toString() for BD6.2 compatibility)
notes:
...
<cffunction access="public" name="getLocalesStruct" output="No" returntype="struct" 
hint="returns struct of locales">

	<cfscript>
		var stLocales = StructNew();
		var locales = getLocales();
		var localeNames = getLocaleNames();

		for(i=1;i LTE ListLen(locales);i=i+1){
			stLocales[ListGetAt(locales,i)] = ListGetAt(localeNames,i);
		}
...
hint="returns list of locales">
	<cfscript>
		...
		for (i=1; i LTE arrayLen(orgLocales); i=i+1) {
			if (listLen(orgLocales[i].toString(),"_") EQ 2) {
				theseLocales=listAppend(theseLocales,orgLocales[i].toString());
...
hint="returns list of locale names, UNICODE direction char (LRE/RLE) added as required">
	<cfscript>
		...
		for (i=1; i LTE arrayLen(orgLocales); i=i+1) {
			if (listLen(orgLocales[i].toString(),"_") EQ 2) {
				if (left(orgLocales[i].toString(),2) EQ "ar" or left(orgLocales[i].toString(),2) EQ "iw")
