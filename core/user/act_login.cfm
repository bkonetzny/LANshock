	...
	<cfelse>
		...
		<cfif NOT ArrayLen(aError)>
			...
			<cfif ListFind(ValueList(qAdmins.id),qLogin.id)>
				...
				<cfinvoke component="#request.lanshock.environment.componentpath#core.admin.admin" method="updateRights">
			...
			<cffile action="append" file="#application.lanshock.environment.abspath#logs/core_user_login.log" output="#cgi.remote_addr# - [#DateFormat(now(),"yyyy-mm-dd")# #TimeFormat(now(),"hh:mm:ss")#] user_#request.session.UserID#, #qLogin.name#">
